...
#==============================================================================
            ...
            elif msg["action"] == "challenged":
                to_name = msg["target"]
                from_name = self.logged_sock2name[from_sock]
                if to_name == from_name:
                    msg = json.dumps({"action": "challenged", "status": "self"})
                # connect to the peer
                elif self.group.is_member(to_name):
                    to_sock = self.logged_name2sock[to_name]
                    self.group.connect(from_name, to_name)
                    the_guys = self.group.list_me(from_name)
                    if len(the_guys) <= 2:
                        msg = json.dumps({"action": "challenged", "status": "success"})
                        for g in the_guys[1:]:
                            to_sock = self.logged_name2sock[g]
                            mysend(to_sock,
                                   json.dumps({"action": "challenged", "status": "request", "from": from_name}))
                    else:
                        self.group.disconnect(from_name)
                        msg = json.dumps({"action": "challenged", "status": "busy"})
                else:
                    msg = json.dumps({"action": "challenged", "status": "no-user"})
                mysend(from_sock, msg)

            elif msg["action"] == "refuse":
                from_name = self.logged_sock2name[from_sock]
                the_guys = self.group.list_me(from_name)[1:]
                for g in the_guys:
                    to_sock = self.logged_name2sock[g]
                    mysend(to_sock, json.dumps({"action": "waiting", "accept": "no", "from": from_name}))
                ...
                for g in the_guys:
                    to_sock = self.logged_name2sock[g]
                    mysend(to_sock,
                           json.dumps({"action": "waiting", "accept": "yes", "from": from_name, "color":msg["color"]}))
